...
class RiskLevel(BaseModel):
    ...
    level: str = Field(description="Risk level: Low/Medium/High (低/中等/高)")
    reason: str = Field(description="Brief explanation for the risk level")
...
class ReportRiskAnalysis(BaseModel):
    ...
    key_policy_attributions: List[PolicyAttribution] = Field(
        description="The 3-5 most critical policies/events determining the overall score and their original text citations (fixed list length of 3)",
        min_length=3, max_length=3
    )
    # Risk assessment field
    risk_assessment: RiskLevel = Field(description="Comprehensive risk level assessment based on sentiment score and signal analysis")

# class ReportRiskAnalysis(BaseModel):
#     """报告风险分析的完整输出结构"""
#     headline: str = Field(description="The title or main subject of the specific article.")
#     category: str = Field(description="e.g., Monetary Policy, Stock Market, Capital flows, Geopolitics, Leisure/Entertainment, Sports.Note that the classification of articles is not limited to the above examples")
#     relevance_score: int = Field(description="0-10 Score: How important is this article for market's sentiment? (e.g., 10=Central Bank/Monetary policy, 0=Movie Review/Music).", ge=0, le=10)
#     sentiment_score: int = Field(description="0-100 Score: The sentiment within this specific article (0=Panic, 50=Neutral, 100=Euphoria).", ge=0, le=100)
#     key_signal: str = Field(description="A concise 1-sentence summary of the market signal extracted from this article.")


# --- Prompt 模板 ---
SYSTEM_PROMPT = (
    "You are an expert Quantitative Financial Analyst specializing in 'Narrative Economics'. "
    "Your task is to process a daily feed of raw news, filter out noise, construct a sentiment index, "
    "and provide a risk level assessment (低/中等/高) based on the analysis."
)
